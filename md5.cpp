...
static uint32_t (*L[4])(uint32_t a, uint32_t b, uint32_t c) = {
...
static uint32_t (*p[4])(uint32_t i) = {
	[](uint32_t i) {return i; },
...
inline static uint32_t rol(uint32_t bits, uint32_t src)
{
...
void static md5_hash(const char *message, uint32_t *v)
{
	uint32_t message_bin[16];
	//for (size_t i = 0; i != 16; ++i)
	//	message_bin[i] = *(uint32_t*)&message[4 * i];
	memcpy(message_bin, message, 64);
...
char *fmd5(const char *filename, char *digest)
{
	...
	while ((n = fread(message, 1, 64, fp)) == 64) {
		md5_hash(message, (uint32_t*)digest);
		size += n;
	}
	fclose(fp);
	size += n;
	...
	++n;
	if (n > 56) {
		memset(message + n, 0, 64 - n);
		md5_hash(message, (uint32_t*)digest);
		n = 0;
	}
	memset(message + n, 0, 56 - n);
	*(uint64_t*)(message + 56) = size * 8;
	md5_hash(message, (uint32_t*)digest);
	return digest;
}
